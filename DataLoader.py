...
class DataLoader:
    def __init__(self, source, target, batch_size, augments):
        ...
        self.src = ['./{}/{}'.format(source, img) for path,
                    subdirs, imgs in os.walk(source) for img in imgs]
        self.src += ['./{}/{}'.format(target, img)
    ...
    def __next__(self):
        ...
        if self.index + self.batch_size >= len(self.src):
            ...
            batch = self.get_batch(
                self.src[self.index: self.index + self.batch_size])
